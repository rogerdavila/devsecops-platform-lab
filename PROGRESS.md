...
## Trivy Operator gap (found + closed 2026-09-18)

Constitution Principles I & II and the Quality Gates table have required Trivy Operator ("continuously scan running workloads for CVEs discovered post-deployment") since v1.0.0 ratification (2026-07-02) — but it was never installed. It fell through the cracks because it's cluster-bootstrap infrastructure (like ArgoCD), not a `platform-info-api` feature task, and the README's "Local Kubernetes cluster" bootstrap section only ever documented ArgoCD, never Trivy Operator, despite both being named as core stack in `CLAUDE.md`/root README from the start. Closed by installing it via the static manifest (`kubectl apply --server-side`, same pattern as ArgoCD, no Helm dependency) and documenting it as step 4 of the cluster bootstrap in `README.md`. Verified live: `VulnerabilityReport` generated for `platform-info-api`'s running pod — 0 CRITICAL, 12 HIGH, 7 MEDIUM, 41 LOW, matching the CI gate (which blocks only CRITICAL).
