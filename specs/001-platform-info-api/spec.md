...
- Q: How should the internal-only restriction (FR-006) be enforced? → A: Defense-in-depth — network-level isolation (NetworkPolicy) AND an application-level check that independently rejects callers not originating from the trusted internal network.
- Q: What dependency does readiness check against in this first release? → A: No real external dependency yet — readiness uses a minimal internal check (e.g., startup initialization complete) as a stand-in for the pattern; adding real external infrastructure is out of scope for this feature.
- Q: What is explicitly out of scope for this feature? → A: End-user authentication, rate limiting/throttling, and any data persistence — this feature is service observability only, not a stateful system.
- Q: What operational metrics must the service expose (FR-005)? → A: Standard HTTP service metrics — request count, latency, error rate, and process uptime.
- Q: What triggers the shutdown behavior described in the edge cases? → A: SIGTERM signal — readiness immediately switches to not-ready while the process stays alive (liveness OK) for a brief drain period before terminating.
...
- What happens when the service is mid-shutdown? Triggered by a SIGTERM signal: readiness MUST switch to not-ready immediately (so the orchestrator stops routing new traffic), while liveness MUST remain healthy for a brief drain period so in-flight requests can complete before the process terminates.
...
- **FR-003**: Readiness MUST reflect the health of the service's dependencies, not just process liveness. In this first release, the only "dependency" is a minimal internal check (e.g., startup initialization complete) — no real external dependency is introduced; this is a deliberate stand-in for the pattern, not a permanent limitation.
- **FR-004**: System MUST expose service metadata (at minimum: version, environment, build identifier) on an internal-only port.
- **FR-005**: System MUST expose operational metrics in a standard, parseable format on an internal-only port, at minimum: request count, request latency, error rate, and process uptime.
- **FR-006**: System MUST restrict access to the metadata and metrics capabilities to callers within the trusted internal network only, enforced at two independent layers: network-level isolation AND an application-level check that independently rejects callers not originating from the trusted internal network (defense-in-depth — neither layer alone is sufficient).
...
- **Operational Metrics**: Represents quantitative runtime measurements of the service, exposed for collection by a monitoring system — at minimum request count, request latency, error rate, and process uptime.
...
- The service has at least one dependency whose health affects readiness; in this first release that dependency is a minimal internal check (e.g., startup initialization complete), not a real external system — introducing real external infrastructure is explicitly out of scope for this feature.
...
- **Out of scope for this feature**: end-user authentication (all consumers are automated systems), rate limiting/throttling on any endpoint, and any form of data persistence — this feature is service observability only, not a stateful system.